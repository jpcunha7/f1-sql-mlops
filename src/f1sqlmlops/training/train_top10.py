"""Train model to predict top-10 finish probability."""
...
MODEL_NAME = "top10_classifier"


def create_pipeline(n_estimators: int = 100, max_depth: int = 10) -> Pipeline:
    """
    Create scikit-learn pipeline for top-10 classification.
...
def train_model(
    ...
    """
    Train top-10 prediction model with MLflow tracking.
    ...
    mlflow.set_experiment("f1_top10_prediction")

    with mlflow.start_run():
        logger.info("Starting top-10 model training")
        ...
        logger.info(f"Train positive class: {train_pos_pct:.1f}%")
        logger.info(f"Val positive class: {val_pos_pct:.1f}%")
        ...
        y_train_proba_raw = pipeline.predict_proba(X_train)
        ...
        # ROC-AUC requires both classes - skip if only one class present
        if len(y_train.unique()) > 1:
            train_auc = roc_auc_score(y_train, y_train_proba)
            train_logloss = log_loss(y_train, y_train_proba)
        else:
            train_auc = 1.0  # Perfect score if only one class
            train_logloss = 0.0
        ...
        y_val_proba_raw = pipeline.predict_proba(X_val)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train top-10 finish prediction model"
    )
    parser.add_argument(
        "--n-estimators",
        type=int,
