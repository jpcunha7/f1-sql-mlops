...
Generated by `make report` in `reports/`:

- **`evidently_top10.html`** - Classification performance report for Top-10 model
  - Accuracy, precision, recall, F1-score
  - ROC curve and confusion matrix
  - Prediction distribution analysis

- **`evidently_dnf.html`** - Classification performance report for DNF model
  - Same metrics as Top-10 report
  - Class balance visualization

- **`evidently_drift.html`** - Data drift analysis
  - Comparison of train vs test feature distributions
  - Statistical drift detection
  - Feature-by-feature drift scores

### Prediction Outputs
- **`predictions/*.csv`** - Race-by-race predictions with probabilities
- **`predictions_summary.txt`** - Human-readable prediction summaries

### MLflow Artifacts
- Experiment runs tracked in `mlruns/`
